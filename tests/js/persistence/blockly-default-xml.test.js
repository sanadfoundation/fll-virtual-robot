...
module.exports = [
  ...
  },
  {
    ...
    fn(_createSim, assert) {
      ...
      const moves = (xml.match(/spike_move_straight/g) || []).length;
      assert.strictEqual(moves, 2, 'expected two spike_move_straight blocks');
      assert.ok(xml.includes('spike_move_steering'), 'expected a steering/turn block');
      assert.ok(xml.includes('spike_light_write'),   'expected a print/light_write block');
    },
  },
];

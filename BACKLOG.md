...
- **Force sensor reads 0 against field walls.** Box2D v2.4 will not generate contacts between the kinematic robot and static walls, so `b2ContactImpulse.normalImpulses` is zero on wall press. The chassis-vs-wall clamp (`js/kinematics.js:clampRobotPose`) stops the robot at the wall but doesn't synthesise a force reading. Fix: when the clamp fires, check whether the bumper fixture's world-space AABB intersects a wall AABB and inject a synthetic impulse into the force-sensor pipeline.

...
- **`motor.absolute_position` accumulator is unbounded.** LEGO docs say `absolute_position` reports degrees in [-180, 179], but the simulator's `_animateTank` / `_animateSingleMotor` accumulate signed wheel rotation without wrapping (`js/simulator.js:1318-1319, 1393`). After three forward turns the Python accessor reads 1080 instead of 0. `run_to_absolute_position` now wraps internally for `direction` math so motion goes the right way, but the reader still returns the raw accumulator. Fix: wrap in the reader (or store wrapped) and update `tests/js/integration/motor-absolute-position.test.js` line 96-98 + `tests/js/integration/motor-encoder.test.js` line 33 — both currently pin the unbounded reading.
